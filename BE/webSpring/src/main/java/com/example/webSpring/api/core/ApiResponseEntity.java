...
public class ApiResponseEntity<T, U> extends ResponseEntity<T> implements Serializable {
    ...
    public ApiResponseEntity(BodyResponse<U> uBodyResponse, HttpHeaders headers, HttpStatusCode statusCode) {
        super((T)uBodyResponse, headers, statusCode);
        this.bodyData = (U) uBodyResponse;
    }
    ...
    public ApiResponseEntity build() {
        return new ApiResponseEntity<>(new BodyResponse<>(getStatusCode().value(), bodyMessage, bodyData), getHeaders(), getStatusCode());
    }
    @Getter
