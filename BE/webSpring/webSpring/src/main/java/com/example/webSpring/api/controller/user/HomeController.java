...
public class HomeController {
    ...
    @ApiOperation(value = "Get list top 3 book selling", response = ApiResponseEntity.BodyResponse.class, tags = Constants.Api.Tag.HOME_MANAGEMENT)
    @ApiResponses(
            value = {
                    @ApiResponse(
                            responseCode = "200",
                            description = "Get list top 3 book selling success",
                            content = @Content(schema = @Schema(implementation = ApiResponseEntity.BodyResponse.class))
                    ),
            }
    )
